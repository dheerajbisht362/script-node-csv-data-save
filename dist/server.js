...
const getDatesInterval_1 = require("./src/helper/getDatesInterval");
const axios_1 = __importDefault(require("axios"));
/**
 api calls to the server and returns
 */
async function apiCall(address, pageSize) {
    ...
    const apiResult = await (0, axios_1.default)(config)
...
async function init() {
    ...
    const apiResult = await apiCall(address, pageSize);
    // write csv file_name
    const filename = "saved_data_from_api.csv";
    const writableStream = fs_1.default.createWriteStream(filename);
    const helperClass = new getDatesInterval_1.HelperClass();
